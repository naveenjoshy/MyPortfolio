document.addEventListener('DOMContentLoaded', () => {
 ...
 // Refactored Form Submission Processing Pipeline
 ...
 if (!isNameValid || !isEmailValid || !isPhoneValid || !isSubjectValid || !isMessageValid) {
   ...
   // 1. Instantly hide the form layout tree completely
   parentForm.style.display = 'none';

   if (successPopup) {
     // 2. Remove 'hidden' and apply flex layout structural visibility
     successPopup.classList.remove('hidden');
     successPopup.classList.add('flex');
     
     // 3. Force browser reflow computation before animating properties
     void successPopup.offsetHeight;
     
     // 4. Smoothly scale and fade your message box into full visibility
     successPopup.classList.remove('opacity-0', 'scale-95');
     successPopup.classList.add('opacity-100', 'scale-100');
     
     // Clear native input cache metrics 
     parentForm.reset();
     visitedFields.clear(); 
   }
 }
